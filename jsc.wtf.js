/*
    @наименование: JavaScript Calendar & Datepicker & Taplendar
    @автор: indapublic
    @комментарии:
    @создан: 2 октября 2012 года
    @изменен: 25 мая 2014 года
*/
...
(function(A) {
    ...
    A.toId = function() {
        ...
        return ((this.getTime() - firstDate.getTime()) / (1000 * 60 * 60 * 24));
    }
...
function Cell(config) {
    ...
    this.defaults = {
        ...
        onCellClick: function(cell, mouseEvent, operation) {},
    ...
    this.render = function() {
        ...
        if (this.element == undefined) {
            ...
            if (this.config.onCellClick instanceof Function)
                container.onclick = this.config.onCellClick.bind(this, this, '3');
...
function Datepicker(config) {
    ...
    config.onCellClick = function(cell, mouseEvent, operation) {
        var
            control = config.control,
            container = config.container,
            calendar = cell.config.calendarInstance;
        if (userCellClick != undefined)
            userCellClick(cell, mouseEvent, operation);
    ...
    config.onCellClick = function(cell, mouseEvent, operation) {
        ...
        switch (cell.config.calendarInstance.mode) {
            ...
            case 'include':
                ...
                break;
            case 'exclude':
                granted = (cell.inScope && (cell.mode == 'include' || cell.mode == 'exclude') && cell.config.calendarInstance.config.editable && !(this.config.calendarInstance.config.nopast && cell.past));
                break;
            default:
                granted = false;
        }
